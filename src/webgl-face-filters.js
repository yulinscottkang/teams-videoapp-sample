...
export class WebglFaceFilters {
  ...
  _op_scale_gl(gl, srcBytes, scale, dstBytes) {
    ...
    if (this.isReady) {
      this._copy3(results, dstWidth, dstHeight);
      this.CVD.draw();
    }
  ...
  _copy3(results, width, height) {
    ...
    JEELIZFACEFILTER.set_inputTexture(tex, width, height);
  ...
  _prepare(w, h) {
    ...
    JEELIZFACEFILTER.init({
      ...
      // NNCPath: "./src/NN_DEFAULT.json",
      // NNCPath: './src/NN_STANDARD_1.json',
      // NNCPath: './src/NN_STANDARD_2.json',
      // NNCPath: './src/NN_LIGHT_1.json',
      NNCPath: './src/NN_VERYLIGHT_1.json',
      // NNCPath: './src/NN_4EXPR_1.json',
      callbackReady: (errCode, spec) => {
        ...
        this.isReady = true;
        this.CVD = JeelizCanvas2DHelper(spec, this._gl, this._canvas);
        this.CVD.ctx.strokeStyle = "yellow";
      }, //end callbackReady()
      ...
      callbackTrack: (detectState) => {
        ...
        if (detectState.detected > 0.001) {
          // draw a border around the face:
          const faceCoo = this.CVD.getCoordinates(detectState);
          console.log(faceCoo);
          this.CVD.ctx.clearRect(
            0,
            0,
            this.CVD.canvas.width,
            this.CVD.canvas.height
          );
          this.CVD.ctx.strokeRect(faceCoo.x, faceCoo.y, faceCoo.w, faceCoo.h);
          this.CVD.update_canvasTexture();
        }
      }, //end callbackTrack()
    });
  }
